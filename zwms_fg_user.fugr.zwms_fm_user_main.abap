...
*"----------------------------------------------------------------------
  ...
  DATA ls_validate_in    TYPE ty_validate_in.
  DATA ls_out            TYPE ty_out.
  DATA ls_validate_out   TYPE ty_validate_out.

  DATA ls_info_in        TYPE ty_info_in.
  DATA ls_info_out       TYPE ty_info_out.

  DATA ls_change_pwd_in  TYPE ty_change_pwd_in.
  DATA ls_change_pwd_out TYPE ty_out.

  DATA ls_update_in      TYPE ty_update_in.
  DATA ls_update_out     TYPE ty_update_out.

  DATA ls_map_in         TYPE ty_map_in.

  CLEAR data_out.

  CASE action.
    WHEN 'CREATE'.
      /ui2/cl_json=>deserialize( EXPORTING json = data_in
                                 CHANGING  data = ls_create_in ).

      CALL FUNCTION 'ZWMS_FM_USER_CREATE'
        EXPORTING  iv_user_id              = ls_create_in-iv_user_id
        ...
        IMPORTING  ev_type                 = ls_out-ev_type
                   ev_msg                  = ls_out-ev_msg
        EXCEPTIONS failed_to_generate_salt = 1
                   OTHERS                  = 2.
      IF sy-subrc <> 0.
        ls_out-ev_type = 'E'.
    ...
    WHEN 'VALIDATE'.
      /ui2/cl_json=>deserialize( EXPORTING json = data_in
                                 CHANGING  data = ls_validate_in ).

      CALL FUNCTION 'ZWMS_FM_USER_VALIDATE'
        EXPORTING iv_user_id  = ls_validate_in-iv_user_id
                  iv_password = ls_validate_in-iv_password
        IMPORTING ev_adm      = ls_validate_out-ev_adm
    ...
    WHEN 'INFO'.
      /ui2/cl_json=>deserialize( EXPORTING json = data_in
                                 CHANGING  data = ls_info_in ).
      CALL FUNCTION 'ZWMS_FM_USER_GET_INFO'
        EXPORTING iv_user_id   = ls_info_in-iv_user_id
                  iv_exact     = ls_info_in-iv_exact
        IMPORTING ev_type      = ls_info_out-ev_type
                  ev_msg       = ls_info_out-ev_msg
        TABLES    et_user_info = ls_info_out-et_user_info.
      IF sy-subrc <> 0.
    ...
    WHEN 'UPDATE_INFO'.
      /ui2/cl_json=>deserialize( EXPORTING json = data_in
                                 CHANGING  data = ls_update_in ).

      CALL FUNCTION 'ZWMS_FM_USER_UPDATE_INFO'
        EXPORTING iv_user_id     = ls_update_in-iv_user_id
                  iv_user_name   = ls_update_in-iv_user_name
                  is_addictional = ls_update_in-is_addictional
        IMPORTING ev_type        = ls_update_out-ev_type
      ...
      data_out = /ui2/cl_json=>serialize( data = ls_update_out ).
      RETURN.

    WHEN 'CHANGE_PWD'.
      /ui2/cl_json=>deserialize( EXPORTING json = data_in
                                 CHANGING  data = ls_change_pwd_in ).

      CALL FUNCTION 'ZWMS_FM_USER_CHANGE_PWD'
        EXPORTING iv_user_id      = ls_change_pwd_in-iv_user_id
        ...
        IMPORTING ev_type         = ls_change_pwd_out-ev_type
    ...
    WHEN 'MAPPING'.
      /ui2/cl_json=>deserialize( EXPORTING json = data_in
                                 CHANGING  data = ls_map_in ).
      CALL FUNCTION 'ZWMS_FM_USER_MAPPING'
        EXPORTING iv_user_id  = ls_map_in-iv_user_id
                  iv_user_map = ls_map_in-iv_user_map
        IMPORTING ev_type     = ls_out-ev_type
                  ev_msg      = ls_out-ev_msg.
      data_out = /ui2/cl_json=>serialize( data = ls_out ).
      RETURN.
