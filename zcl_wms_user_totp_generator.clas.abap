CLASS zcl_wms_user_totp_generator DEFINITION
  PUBLIC FINAL
  CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS get_instance
      IMPORTING im_secret_key            TYPE string
                im_otp_len               TYPE char2 DEFAULT '6'
                im_validity_sec          TYPE char2 DEFAULT '30'
      RETURNING VALUE(ro_totp_generator) TYPE REF TO zcl_wms_user_totp_generator.

    METHODS get_totp
      RETURNING VALUE(r_totp) TYPE string
      RAISING   cx_demo_dyn_t100.
  ...
  PRIVATE SECTION.
    TYPES:
      BEGIN OF ty_bin2hex,
        ...
        hex TYPE char1,
      END OF ty_bin2hex.
    TYPES tt_bin2hex TYPE STANDARD TABLE OF ty_bin2hex WITH EMPTY KEY.
    ...
    TYPES:
      BEGIN OF ty_hex2dec,
        ...
        dec TYPE char2,
      END OF ty_hex2dec.
    TYPES tt_hex2dec TYPE STANDARD TABLE OF ty_hex2dec WITH EMPTY KEY.

    CLASS-DATA o_totp_generator TYPE REF TO zcl_wms_user_totp_generator.
    ...
    METHODS base32_to_hex
      IMPORTING im_secret_key  TYPE string
      EXPORTING ex_hexadecimal TYPE string
                ex_rc          TYPE i.

    METHODS conv_dec_to_bin
      IMPORTING im_decimal     TYPE i
                im_base32_flag TYPE abap_bool DEFAULT abap_false
      RETURNING VALUE(rv_bit)  TYPE string.
...
CLASS zcl_wms_user_totp_generator IMPLEMENTATION.
  METHOD constructor.
    t_bin2hex = VALUE #( ( bin = '0000' hex = '0' )
                         ( bin = '0001' hex = '1' )
                         ( bin = '0010' hex = '2' )
                         ( bin = '0011' hex = '3' )
                         ( bin = '0100' hex = '4' )
  ...
  METHOD get_totp.
    ...
    TRY.
        ...
        " Finally collecting HMAC Hex data
  ...
  METHOD conv_dec_to_bin.
    ...
    DATA l_divisible TYPE i.
    DATA l_bit       TYPE string.
    DATA l_bitn      TYPE numc5.
  ...
  METHOD conv_epoch_to_hex.
    ...
    SELECT SINGLE * FROM ZWMS_T_TOTPLOG
        INTO @DATA(lwa_ZWMS_T_TOTPLOG)
        WHERE userid = @sy-uname.
    IF sy-subrc <> 0.
      l_epoch = cal_epoch_unix_time( ).
      l_hex  = conv_dec_to_hex( im_decimal = l_epoch ).

      lwa_ZWMS_T_TOTPLOG-userid = sy-uname.
      lwa_ZWMS_T_TOTPLOG-epoch = l_epoch .
      lwa_ZWMS_T_TOTPLOG-timestamp = me->g_lastrun_timestmp.
      INSERT ZWMS_T_TOTPLOG FROM lwa_ZWMS_T_TOTPLOG.
      COMMIT WORK.
    ELSE.
      IF ( l_tstamp - lwa_ZWMS_T_TOTPLOG-timestamp ) GT me->w_validity_sec.
        l_epoch = cal_epoch_unix_time( ).
        l_hex  = conv_dec_to_hex( im_decimal = l_epoch ).
        ...
        lwa_ZWMS_T_TOTPLOG-epoch = l_epoch .
        lwa_ZWMS_T_TOTPLOG-timestamp = me->g_lastrun_timestmp.
        MODIFY ZWMS_T_TOTPLOG FROM lwa_ZWMS_T_TOTPLOG.
        COMMIT WORK.
      ELSE.
        l_hex  = conv_dec_to_hex( im_decimal = lwa_ZWMS_T_TOTPLOG-epoch ).
      ENDIF.
    ENDIF.
