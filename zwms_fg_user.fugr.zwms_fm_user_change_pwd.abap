...
*"     VALUE(IV_TOTP_VALID) TYPE  CHAR1
*"  EXPORTING
...
*"----------------------------------------------------------------------
  ...
  AND iv_new_password IS INITIAL.
    ev_type = 'E'.
  ...
  data(lv_request_id) = to_upper( iv_request_id ).
  ...
  select single
    ...
    where user_id = @lv_request_id
    into @data(lv_is_map_account).
  ...
  DATA(is_adm_acct) = xsdbool( lv_is_sap_account = abap_true OR
                               lv_is_adm_account = abap_true or
                               lv_is_map_account = abap_true or
                               iv_totp_valid     = abap_true ).
