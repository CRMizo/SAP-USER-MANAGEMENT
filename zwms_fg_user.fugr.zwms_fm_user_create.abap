...
*  " Check SAP Standard User Binding status
*  IF is_addictional-sap_user IS NOT INITIAL.
*    DATA(lv_sap_user) = to_upper( is_addictional-sap_user ).
*
*    SELECT SINGLE
*           bname,
*           erdat
*      FROM usr02
*      WHERE bname = @lv_sap_user
*      INTO @DATA(lv_sap_user_exist).
*    IF sy-subrc <> 0.
*      ev_type = 'E'.
*      ev_msg  = 'SAP account does not exist.'.
*      RETURN.
*    ENDIF.
*
...
*      INTO @DATA(lv_user_mapping_exist).
*    IF lv_user_mapping_exist = abap_true.
*      ev_type = 'E'.
*      ev_msg  = 'The current SAP account has already been bound.'.
*      RETURN.
*    ENDIF.
*  ENDIF.
...
*  ls_user_map = VALUE #( sap_user = lv_sap_user
*                         user_id  = iv_user_id ).
*  TRY.
